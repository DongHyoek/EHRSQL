...
class Build_eICU(Sampler):
                ...
                for visit_seq, hos_id in enumerate(pt_data_given_same_year["patienthealthsystemstayid"].unique()):
                    ...
                    for icu_seq, icu_id in enumerate(hadm_data["patientunitstayid"].unique()):
                        ...
                        if hospitaladmitoffset <= unitadmitoffset and unitadmitoffset < unitdischargeoffset and unitdischargeoffset <= hospitaldischargeoffset:
                            ...
                            self.unitstayid2time_dict[icu_id] = str(
                                hospitaladmittime24_shifted - timedelta(hours=0, minutes=int(hospitaladmitoffset))
                            )  # sampled_hosp_admit - (icu_admit offset from hosp_admit) => sampled_icu_admit
                            cnt_processed += 1
    ...
    def build_intakeoutput_table(self):
        ...
        intakeoutput_table["intakeoutputtime"] = adjust_time(
            intakeoutput_table, "intakeoutputoffset", current_time=self.current_time, offset_dict=self.unitstayid2time_dict, patient_col="patientunitstayid"
        )
    ...
    def build_vital_table(self):
        ...
        vital_table = read_csv(
            ...
            columns=["vitalperiodicid", "patientunitstayid", "observationoffset", "temperature", "sao2", "heartrate", "respiration", "systemicsystolic", "systemicdiastolic", "systemicmean"],
            lower=True,
